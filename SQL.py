...
class SQL():
    ...
    def get_all_books(self):
        return self.db.session.query(Book.my_id, Book.counter_id, Book.name, Book.is_marked).all()

    def get_books_by_id(self, id):
        return self.db.session.query(Book.counter_id, Book.name, Book.is_marked).filter(Book.my_id == id).all()

    def get_bookmark_by_id(self, id):
        return self.db.session.query(Book.counter_id, Book.name, Book.is_marked).\
            filter(Book.my_id == id, Book.is_marked).all()
